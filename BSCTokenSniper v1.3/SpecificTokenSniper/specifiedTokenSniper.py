...
global pancakeSwapRouterAddress
global pancakeSwapFactoryAddress
...
try:
    ...
    pancakeSwapFactoryAddress = configData['pancakeSwapFactoryAddress']
    ...
    antiBotDelay = float(configData['antiBotDelay'])

except:
    print(style.RED + "FATAL ERROR: Incorrectly configured config.json file.")
    input(style.RED + "Please check that it syntactically correct with no missing/extra colons, quotation marks, curly brackets or omitted entries, or that data types are correct, and restart the program.")
    exit()
...
try:
    ...
    print(style.RED + "FATAL ERROR: Cannot connect to BSC node. This could be because your internet connection is unavailable, the BSC node has connectivity issues or a firewall is blocking the program's access.")
    input(style.RED + "Please fix the error and restart the program.")
    exit()
...
def updateTitle():
    ...
    if(sys.platform == "win32"): #if os is windows
        ctypes.windll.kernel32.SetConsoleTitleW("BSCTokenSniper v1.3 | Tokens Detected: " + str(numTokensDetected) + " | Tokens Bought: " + str(numTokensBought) + " | Wallet Balance: " + str(walletBalance) + " BNB")
    else:
        sys.stdout.write("BSCTokenSniper v1.3 | Tokens Detected: " + str(numTokensDetected) + " | Tokens Bought: " + str(numTokensBought) + " | Wallet Balance: " + str(walletBalance) + " BNB")
        sys.stdout.flush()
...
targetContractAddress = input("Enter target contract address: ")
...
def Buy(tokenAddress, tokenSymbol):
    try:
        if(tokenAddress != None):
            ...
            try:               
                if(antiBotDelay > 0):
                    print(style.YELLOW + currentTimeStamp + " [Token] Delaying purchase for " + antiBotDelay + " seconds for antibot delay.")
                    time.sleep(antiBotDelay)
                ...
                print(style.RED + currentTimeStamp + " Transaction failed: " + ex)
        ...
        #TOKEN IS BOUGHT
            ...
            txResult = checkTransactionRequest.json()['result']['status']
        ...
        print(style.RED + "[ERROR] Please notify dev: ")
        print(style.RED + traceback.format_exc())
    ...
    try:
        ...
        if(jsonEventContents['args']['token0'] == liquidityPairAddress or jsonEventContents['args']['token1'] == liquidityPairAddress): #check if pair is WBNB, if not then ignore it
            ...
            if jsonEventContents['args']['token1'] == liquidityPairAddress:
                ...
                bnbPairIndex = 0
            #print("bnbpairindex: ", bnbPairIndex)
           ...
           # print("got tokenname and symbol")
           ...
           # print(currentTimeStamp + " In the middle of processing token. foundTokenThreadBusy? ", foundTokenThreadBusy)
    ...
    while True:
        try:
            for PairCreated in event_filter.get_new_entries():
                #print(currentTimeStamp + "Found new token. foundTokenThreadBusy? ", foundTokenThreadBusy)
                if foundTokenThreadBusy == False:
                    foundToken(PairCreated)
                else:
                    print(currentTimeStamp + " Main token thread busy, ", foundTokenThreadBusy)
                    while foundTokenThreadBusy:
                        pass #waiting for when foundToken thread is busy
                    foundToken(PairCreated)
...
def listenForTokens():
    #print("listenfortokens start")
    event_filter = contract.events.PairCreated.createFilter(fromBlock='latest')
    #block_filter = web3.eth.filter('latest')
    # tx_filter = web3.eth.filter('pending')
    loop = asyncio.get_event_loop()
    try:
        #print("listenfortokens running")
