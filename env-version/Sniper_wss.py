...
pancakeSwapRouterAddress = os.environ['pancakeSwapRouterAddress'] # .env
pancakeSwapFactoryAddress = os.environ['pancakeSwapFactoryAddress'] # .env
walletAddress = os.environ['walletAddress'] # .env
private_key = os.environ['walletPrivateKey'] # .env
snipeBNBAmount = float(os.environ['amountToSpendPerSnipe'])
transactionRevertTime = int(os.environ[
                                'transactionRevertTimeSeconds'])  # number of seconds after transaction processes to cancel it if it hasn't completed
gasAmount = int(os.environ['gasAmount'])
gasPrice = int(os.environ['gasPrice'])
bscScanAPIKey = os.environ['bscScanAPIKey']
observeOnly = os.environ['observeOnly']

checkSourceCode = os.environ['checkSourceCode']
checkValidPancakeV2 = os.environ['checkValidPancakeV2']
checkMintFunction = os.environ['checkMintFunction']
checkHoneypot = os.environ['checkHoneypot']
checkPancakeV1Router = os.environ['checkPancakeV1Router']
checksolidity4 = os.environ['checksolidity4']
...
def updateTitle():
    ...
    sys.stdout.write("BSCTokenSniper | Tokens Detected: " + str(numTokensDetected) + " | Tokens Bought: " + str(
        numTokensBought) + " | Wallet Balance: " + str(walletBalance) + " BNB")
    sys.stdout.flush()
...
def Buy(tokenAddress, tokenSymbol):
    if (tokenAddress != None):
        ...
        if (txResult == "1"):
            print(style.GREEN + currentTimeStamp + " Successfully bought $" + tokenSymbol + " for " + style.BLUE + str(
                snipeBNBAmount) + style.GREEN + " BNB - TX ID: ", txHash)
...
def foundToken(event):
    try:
        ...
        if (jsonEventContents['args'][
            'token1'] == "0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c"):  # check if pair is WBNB, if not then ignore it
            ...
            print(
                style.YELLOW + currentTimeStamp + " New potential token detected: \n " + style.CYAN + "Token name : " + tokenName + " (" + tokenSymbol + "): " + style.MAGENTA + "\n Contract   : " + tokenAddress + style.RESET)
